...
public class PhotonConnectionManager : MonoBehaviourPunCallbacks, IInRoomCallbacks
{
    ...
    {
        Debug.Log("quyen quyenn" + cause);
        switch (cause)
        {
            case DisconnectCause.DisconnectByClientLogic:
                if (IsDisConnectedByHost)
                {
                    Toast.Show(MeetingConfig.hostLeftMessage, MeetingConfig.longToastDuration);
                }
                else
                {
                    Toast.Show(MeetingConfig.finishingMeeting, MeetingConfig.longToastDuration);
                }
                SceneManager.LoadScene(SceneNameManager.prevScene);
            ...
            // Debug.Log("quyen quyenn" + IsReConnectedAndRejoned);
            //     if (!IsReConnectedAndRejoned)
            //     {
            //         IsReConnectedAndRejoned = PhotonNetwork.ReconnectAndRejoin();
            ...
            //     }  
                ...
                {
                    Debug.Log("quyen Waiting for client to be fully disconnected..");

                    yield return new WaitForSeconds(0.2f);
                }

                Debug.Log("quyen Client is disconnected!");

                if (!PhotonNetwork.ReconnectAndRejoin())
                {
                    if (PhotonNetwork.Reconnect())
                    {
                        Debug.Log("quyen Successful reconnected!");
                    }
                }
                else
                {
                    Debug.Log("quyen Successful reconnected and joined!");
                }
                break;
            default:
                break;
